...
/** Opencode `.opencode/plugin/vf-guard.ts` — NATIVE enforcement via the plugin
 *  system. Opencode auto-loads any `*.ts` / `*.js` under `.opencode/plugin/` and
...
export function opencodePluginSource(): string {
  ...
  return `// VibeFlow guardrail plugin for opencode (auto-loaded from .opencode/plugin/).
...
export function engineHookFiles(engines?: Engine[]): Record<string, string> {
  return {
    ...
    ...(!engines || engines.includes("opencode")
      ? { ".opencode/plugin/vf-guard.ts": opencodePluginSource() }
      : {}),
