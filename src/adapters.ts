...
export function defaultContext(): ProjectContext {
  return {
    ...
    // Defaults that satisfy canonicalFiles()'s fail-closed contract. Callers
    // SHOULD override these with real values from PROJECT_CONTEXT.md or
    // REQUIREMENTS.md; the defaults are the bare minimum that lets the
    // function return something usable without throwing.
    docSource: `${CTX_DIR}/PROJECT_CONTEXT.md`,
    taskSource: `${CTX_DIR}/TASK_CONTEXT.md`,
    expectedResult: "All vf verify gates pass (typecheck, lint, test, build, smoke).",
  };
}
...
export function canonicalFiles(ctx: ProjectContext): Record<string, string> {
  // Fail-closed on missing required fields. Previously, missing values
  // were silently filled with "TODO" placeholders, which would ship into
  // the AI agent's context and be treated as ground truth.
  if (!ctx.docSource) {
    throw new Error("canonicalFiles: docSource is required (no TODO fallback)");
  }
  if (!ctx.taskSource) {
    throw new Error("canonicalFiles: taskSource is required (no TODO fallback)");
  }
  if (!ctx.expectedResult) {
    throw new Error("canonicalFiles: expectedResult is required (no TODO fallback)");
  }
  const sources = [
    `- Documentation source: ${ctx.docSource}`,
    `- Task/issue source: ${ctx.taskSource}`,
    ctx.fileTypes?.length ? `- File types in scope: ${ctx.fileTypes.join(", ")}` : null,
  ]
    .filter(Boolean)
    .join("\n");
  const requirements = `- Expected result: ${ctx.expectedResult}\n`;
  ...
  return {
    ...
    [`${CTX_DIR}/TASK_CONTEXT.md`]: `# Task Context\n\n- Goal: ${ctx.goal}\n- Definition of Done: ${ctx.expectedResult}\n- Must not change: (preserved by user — provide explicit scope before editing)\n`,
