...
//   (B) notifyUpdate()     — passive 1-line nudge printed on any command, driven
//                             from a 24h cache so it adds ZERO latency; refreshes
 ...
 *  any network error, non-2xx, or malformed body yields `null` (never throws). */
...
/** Read the cache. `null` when absent, unreadable, or malformed. */
...
/** Passive nudge: print "update available" from the cache (zero latency), and
