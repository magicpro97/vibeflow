...
// Re-export every type moved to core/types.ts so 86 importers are unchanged.
export type {
  Engine,
  GateState,
  WorkUnit,
 ...
 * On FIRST creation, tightens the file to 0o600 like writeFileSafe (#536): the append targets —
 * guidance notes (sensitive steering text) and the work journal — may carry private context and
 * must not land world-readable at umask-default 0644. The chmod is gated on creation so the
 * per-SSE-chunk stream.log appends (dispatch-runtime) don't pay a redundant chmod syscall on
 * every write; the mode is already 0o600 after the first append. */
