...
export function readStableConversationJournal(
  ...
  return readStableConversationJournalAttempt(initialSnapshot, conversationId, 0);
}

function readStableConversationJournalAttempt(
  ...
  fs.closeSync(snapshot.fd);
  return readStableConversationJournalAttempt(next, conversationId, attempt + 1);
}
