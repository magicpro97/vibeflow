 ...
 *  #533: this splits on single spaces (`cmd.split(" ")`) and runs
 *  `spawnSync(bin, args)` with NO `shell:true`, so shell metacharacters (pipes,
 *  quotes, redirects, `$()`, globs) are passed as LITERAL args, not interpreted.
