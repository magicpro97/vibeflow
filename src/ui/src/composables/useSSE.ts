...
export function useSSE(url: string, maxLines = 500) {
  ...
  async function catchup() {
    try {
      ...
      let since = 0;
      try {
        const sess = await fetch("/api/logs/session");
        if (sess.ok) {
      ...
      if (res.ok) {
        const { events } = (await res.json()) as { events: LogEvent[] };
        for (const ev of events) pushSorted(ev);
      }
    } catch (_e) {
  ...
  function connect() {
    ...
    es.addEventListener("log", (e) => {
      ...
      error.value = null;
      try {
        push(JSON.parse((e as MessageEvent).data) as LogEvent);
      } catch (_e) {
