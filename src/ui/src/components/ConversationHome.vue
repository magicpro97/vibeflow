      ...
      <header v-if="store.activeSession" class="home-conversation-header">
        <div>
          <div class="home-header-badges">
            <span class="home-header-state" :data-health="store.activeRevision?.health">
              {{ lifecycleLabel }}
            </span>
            <span
              v-if="streamLabel"
    ...
    <Transition name="home-inspector">
      ...
      >
        ...
        <section>
          ...
          <button class="home-inspector-link" type="button" @click="openSecondary('open-capabilities')">CLI capabilities <span>→</span></button>
        </section>
      </aside>
    </Transition>
  </div>
...
const lifecycleLabel = computed(() =>
  (store.activeRevision?.lifecycle ?? "ACTIVE")
    .toLowerCase()
