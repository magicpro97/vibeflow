...
## [2026-06-13] hardening round-1 + round-2 + round-3 (PR #27)

Round-1 (16 commits): initial hardening stack — deny-by-default env allowlist, assertWithinRoot for rmSync guards, argv-form spawn defaults, runSemgrep integration with custom vibeflow rules, esc() XSS extraction + sync test, fail-closed canonical context, 2-pass realpath resolution.

Round-2 (6 commits): cross-debate caught (B3+stealth B4) sync runDispatch still using /bin/sh -c + unfiltered env; added PASSTHROUGH set for SSH_AUTH_SOCK + Windows plumbing; bounded realpath walk to MAX_REALPATH_DEPTH=4096; -1 sentinel for runSemgrep parse error; tokenizeVibeflowAi across sync/async/aiGenerate (BREAKING feat!); replaced typeof-only tests in ai-init-shell.test.ts with real cat-roundtrip + marker-file side-effect assertions.

Round-3 (8 commits): rebase onto current main; aiEnrichRole (B3 reopen on main) fixed; runSemgrep extracted decideSemgrepResult pure function + r.status===null fix; aiGenerate + aiEnrichRole now pass env: filterChildEnv; importer.ts assertWithinRoot moved BEFORE cpSync; dispatch-shell.test.ts line 11,17 typeof-only replaced with real spawn; escape-sync.test.ts trust model documented; safety/path.ts depth-cap + Windows cross-drive limitations documented.

Gates: tsc clean, biome 104 files clean, 650 pass / 2 skip / 3 fail (3 skill-sync test fail pre-existing from main's tightened SKILL.md body validation, out of scope).

Total PR: 28+ commits, 26+ files, +1300/-200. Bumping release-please major because feat! + multiple B3/B4 safety fixes.
