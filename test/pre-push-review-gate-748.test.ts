...
describe("generated pre-push hook (#748)", () => {
  ...
  test("engine map exposes a vibeflow-managed pre-push that gates current HEAD", () => {
    expect(engineHookFiles()[".githooks/pre-push"]).toBeDefined();
    const sh = gitPrePush();
    expect(sh).toContain("# vibeflow-managed");
    expect(sh).toContain("verify --require-review-evidence --review-base");
    expect(sh).toContain("git push --no-verify");
  });

  test("existing-branch push uses remote sha as review base; valid verify permits", () => {
    const r = runHook(dir, [`refs/heads/main ${h} refs/heads/main ${baseFoo}`], 0);
    expect(r.status).toBe(0);
    expect(argsFor(dir)).toEqual(["verify", "--require-review-evidence", "--review-base", baseFoo]);
  });

  test("non-zero verify blocks the push", () => {
  ...
  test("ignores tags and deletions; gates only the pushed branch", () => {
    ...
    expect(r.status).toBe(0);
    expect(argsFor(dir)).toEqual(["verify", "--require-review-evidence", "--review-base", baseBar]);
  });
